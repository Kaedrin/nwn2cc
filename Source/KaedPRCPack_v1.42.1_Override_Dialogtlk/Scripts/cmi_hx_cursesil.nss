...
void main()
{
   ...
   */
	...
	float fKnockdown = 6.0f;

    eLink = SetEffectSpellId(eLink, SPELLABILITY_HEX_CURSE_SIL);
    ...
    while(GetIsObjectValid(oTarget2))
    {
        ...
        {
			SignalEvent(oTarget2, EventSpellCastAt(OBJECT_SELF, SPELLABILITY_HEX_CURSE_SIL)); 
			if (!MySavingThrow(SAVING_THROW_WILL, oTarget2, GetHexbladeDC(nLevel/2), SAVING_THROW_TYPE_ALL))
			{
				...
				if (nEpic)
					DelayCommand(0.1f, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eKnockdown, oTarget2, fKnockdown));					
				nAffected++;
			}
        }
        oTarget2 = GetNextObjectInShape(SHAPE_SPHERE, RADIUS_SIZE_COLOSSAL, GetLocation(oTarget));
    }	

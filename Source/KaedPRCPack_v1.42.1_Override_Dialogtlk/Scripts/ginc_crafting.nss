...
int GetIsItemPropertyAnUpgrade(object oItem, itemproperty ip, int nDurationType = -1)
{
	...
	// FIX: Also ignore properties that have no subtype, otherwise false negative can happen
...
void CreateListOfItemsInInventory(string sItemTemplateList, object oTarget, int bIdentify=TRUE)
{
	...
	while (sItemTemplate != "")
	{
		...
		int iAmmo = FindSubString(sItemTemplate,"cmi_ammo");
		int iThrown = FindSubString(sItemTemplate,"cmi_thrown");
		int nAmmo = 0;
		
		if ( FindSubString(sItemTemplate,"wamar") > 0)
			nAmmo = 1;
		if ( FindSubString(sItemTemplate,"wambo") > 0)
			nAmmo = 1;		
		if ( FindSubString(sItemTemplate,"wambu") > 0)
			nAmmo = 1;	

			
		//SendMessageToPC(GetFirstPC(), "Template: " + GetStringUpperCase(sItemTemplate));		
			
		int nAmmoStacks = StringToInt(Get2DAString("cmi_options","Value",CMI_OPTIONS_AmmoStacksToCreate));	
		if (nAmmoStacks > 5)
			nAmmoStacks = 5;
		...
		{
			int iCount = 1;
			for (iCount = 1; iCount <= nAmmoStacks; iCount++)
			{
				oCreatedObject = CreateItemOnObject(sItemTemplate, oTarget,99);
			}
		}
		else
		if (iThrown >= 0)
		{
			...
			for (iCount = 1; iCount <= nAmmoStacks; iCount++)
			{
				oCreatedObject = CreateItemOnObject(sItemTemplate, oTarget,50);
			}			
		}	
		else
		if ( nAmmo )
...
int GetPropSlotsUsed(object oItem)
{
	...
	{
		int iVal = GetItemBaseMaterialType(oItem);
		if (iVal == 1 || iVal == 3 || iVal == 5 || iVal == 7) //Adam, Dark, Mith, Dusk
			iItemPropCount--;
	}
	else
	if (GetIsArmorOrShield(oItem))
	{
		...
		{
			if (iVal == 3 || iVal == 7 || iVal == 8 || iVal == 11) //Dark, Dusk, Umber, Zal
				iItemPropCount--;	
			if (iVal == 5) //Mith
				iItemPropCount = iItemPropCount - 2;				
		}
		else
		{
			if (iVal == 3 || iVal == 11) //Dark, Umber
				iItemPropCount--;	
			if (iVal == 9 || iVal == 10 || iVal == 12) //Salamander, Red Dragon, Wyvern
				iItemPropCount = iItemPropCount - 2;			
		}
	}
...
void DoMagicCrafting(int iSpellID, object oPC)
{
	...
	{
		nCasterLevel2 = GetWarlockCasterLevel(oPC);
	}
	if (GetLevelByClass(CLASS_TYPE_RANGER, oPC) > 0 || GetLevelByClass(CLASS_TYPE_PALADIN, oPC) > 0)
	{
		nCasterLevel2 = GetPalRngCasterLevel(oPC);	
	}
	if (nCasterLevel2 > nCasterLevel)
		nCasterLevel = nCasterLevel2;

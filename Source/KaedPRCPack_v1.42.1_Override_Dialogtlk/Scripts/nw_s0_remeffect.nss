...
void SafeRemoveAbilityDecrease(object oTarget)
{
    ...
    while (GetIsEffectValid(eAOE))
    {
        if ((GetEffectType(eAOE) == EFFECT_TYPE_ABILITY_DECREASE) &&
            ...
            GetEffectSpellId(eAOE) != SPELL_REDUCE_PERSON &&
            GetEffectSpellId(eAOE) != SPELL_REDUCE_ANIMAL &&
            GetEffectSpellId(eAOE) != SPELL_REDUCE_PERSON_GREATER &&
            GetEffectSpellId(eAOE) != SPELL_REDUCE_PERSON_MASS &&
...
void SafeRemoveAbilityDecrease2(object oTarget)
{
    //Declare major variables
    //Get the object that is exiting the AOE
    effect eAOE;
    ...
    while (GetIsEffectValid(eAOE))
    {
        if ((GetEffectType(eAOE) == EFFECT_TYPE_ABILITY_DECREASE) &&
            ...
            GetEffectSpellId(eAOE) != SPELL_REDUCE_PERSON_GREATER &&
            GetEffectSpellId(eAOE) != SPELL_REDUCE_PERSON_MASS &&
			GetEffectSpellId(eAOE) != SPELLABILITY_HEX_CURSE &&			
            GetEffectSpellId(eAOE) != 803)
        {
            //If the effect was created by the spell then remove it
            RemoveEffect(oTarget, eAOE);
    ...
    if(nEffect2 != 0)
    {
        if (nEffect2 == EFFECT_TYPE_ABILITY_DECREASE)
        {
			 if(nSpellID == SPELL_REMOVE_DISEASE || nSpellID == SPELLABILITY_REMOVE_DISEASE)
				SafeRemoveAbilityDecrease2(oTarget);	
			 else

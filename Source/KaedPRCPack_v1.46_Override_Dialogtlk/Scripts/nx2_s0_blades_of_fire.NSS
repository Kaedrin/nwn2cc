...
//	SpawnScriptDebugger();
	...
	int nLastCastClass = GetLastSpellCastClass();
	if (nLastCastClass == CLASS_TYPE_RANGER)
	{
		fDuration = RoundsToSeconds(GetPalRngCasterLevel());
	}	
	
	fDuration			= ApplyMetamagicDurationMods(fDuration);

	// Effects
	int ipDmg = IP_CONST_DAMAGEBONUS_1d8;
	if (GetHasFeat(FEAT_WILDFIRE_BLADES))
		ipDmg = IP_CONST_DAMAGEBONUS_1d10;
	
	itemproperty ipFire 	= ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_FIRE, ipDmg);

...
void main()
{
    ...
    int nWarpriestLevel = GetLevelByClass(CLASS_TYPE_WARPRIEST);
	int nMoRLevel = GetLevelByClass(CLASS_MASTER_RADIANCE);
    int nDoomguideLevel = GetLevelByClass(CLASS_TYPE_DOOMGUIDE);
	int nShadStalkerLevel = GetLevelByClass(CLASS_SHADOWBANE_STALKER);
...
// JLR - OEI 06/21/05 NWN2 3.5
	...
	int nVengTaker = GetLevelByClass(CLASS_VENGTAKE);
	nClassLevel += nVengTaker;
	nTurnLevel += nVengTaker;	
	...
	if (nUseAlternateTurnUndeadRules 
		&&
		 (
			(nPaladinOnlyAlternateTurnUndeadRule == 0) ||
			((nPaladinOnlyAlternateTurnUndeadRule == 1) && (nPaladinLevel > 3))
		 ) 
	   )
	{
		//Do Damage Routine
	    ...
	    while( GetIsObjectValid(oTarget) && nHDCount < nTurnHD )
	    {
	        if( spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF) && oTarget != OBJECT_SELF )	// AFW-OEI 06/28/2006: Hopefully, the spellsIsTarget finally excludes all friendlies
	        {
				...
				*/
				...
				else if (nRacial == RACIAL_TYPE_UNDEAD || (nRacial == RACIAL_TYPE_OUTSIDER && nGoodOrEvilDomain+nPlanar > 0) || (nRacial == RACIAL_TYPE_CONSTRUCT && nConstructs > 0) || (nRacial == RACIAL_TYPE_ELEMENTAL && nElemental > 0) || (nRacial == RACIAL_TYPE_VERMIN && nVermin > 0) )
				{

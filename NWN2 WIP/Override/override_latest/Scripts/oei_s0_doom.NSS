...
// End of Spell Cast Hook
    ...
    int nLevel;
	int nSpellDC;	
    if (GetSpellId() == SPELL_BG_Doom)
	{	
        nLevel = GetBlackguardCasterLevel(OBJECT_SELF);
		nSpellDC = oei_GetSpellSaveDC(ABILITY_WISDOM);
	}
	else
	{
    	nLevel = GetCasterLevel(OBJECT_SELF);
    ...
    if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
    {
        ...
        {
	        int nResult =       WillSave(oTarget, nSpellDC, SAVING_THROW_TYPE_MIND_SPELLS);
	        if (nResult == 2)
	        {

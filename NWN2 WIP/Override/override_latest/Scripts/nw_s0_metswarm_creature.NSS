...
int DetermineFireDamageForTargetCreature(object oTarget, int nTouchAttackRanged)
{
	...
	if (nMetaMagic == METAMAGIC_EMPOWER)
	{
		nDamage = nDamage + (nDamage/2); //Damage/Healing is +50%		
					...
					int nSaveThrowType = SAVING_THROW_TYPE_FIRE;			
					int nLastClass = GetLastSpellCastClass();
					if (nLastClass == CLASS_TYPE_DRUID || nLastClass == CLASS_TYPE_SPIRIT_SHAMAN)
					{
						nSaveThrowType = SAVING_THROW_TYPE_COLD;
					}
					
	if (nTouchAttackRanged < 1)
	{
		nDamage = GetReflexAdjustedDamage(nDamage, oTarget, GetSpellSaveDC(),nSaveThrowType);
	}
...
//	int nTouchAttack = TouchAttackRanged(oTarget, TRUE); 	
	...
	effect eFire = EffectDamage(nDamage, nDmgType, DAMAGE_POWER_ENERGY);
	return eFire;
}

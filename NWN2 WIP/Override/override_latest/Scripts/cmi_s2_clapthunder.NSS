...
int GetSonicReserveDamageDice()
{
	...
	if ( GetHasSpell(1038) || GetHasSpell(1820) || GetHasSpell(1914) )
		return 8;
	if ( GetHasSpell(445) ||  GetHasSpell(512) )
		return 6;
...
void main()
{
	...
	effect eOrbEffect = EffectDeaf();
	int nSaveThrow = SAVING_THROW_TYPE_SONIC;
	...
	if (GetIsObjectValid(oTarget))
	{
		...
		{		
			...
			if (nMeleeTouch != TOUCH_ATTACK_RESULT_MISS)
			{
				...
				effect eDamage = EffectDamage(nDamage,DAMAGE_TYPE_SONIC, DAMAGE_POWER_NORMAL, bIgnoreResists);
                ...
                if  (!MySavingThrow(SAVING_THROW_FORT, oTarget, nDC, nSaveThrow))
					DelayCommand(0.3f, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eStatusEffect, oTarget, 6.0f));				

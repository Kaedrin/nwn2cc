    ...
    while (GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
    	{
            ...
            if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
    	    {
				...
				if (nHasPierceCold)
				{
					nDamage2 = AdjustPiercingColdDamage(nDamage2, oTarget);
				}
				
                eDam2 = EffectDamage(nDamage2, nDmgType, DAMAGE_POWER_NORMAL, nHasPierceCold);
                // Apply effects to the currently selected target.
                DelayCommand(0.0f, ApplyEffectToObject(DURATION_TYPE_INSTANT, eDam, oTarget));
                if (nDmgType != DAMAGE_TYPE_MAGICAL)

...
//:: RPGplayer1 12/20/2008: For Fey Legacy, use HD as caster level
...
// End of Spell Cast Hook
	...
	while (GetIsObjectValid(oTarget))
	{
        if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
    	{
		   ...
		   if (!GetIsImmune(oTarget, IMMUNITY_TYPE_CONFUSED, OBJECT_SELF))
		   {
	           ...
	           if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
	    	   {
	                ...
	                if (!MySavingThrow(SAVING_THROW_WILL, oTarget, GetSpellSaveDC(), SAVING_THROW_TYPE_MIND_SPELLS, OBJECT_SELF, fDelay))
	                {
	                   ...
	                   //Has same SpellId as Confusion, not an item, but returns no valid class -> it's Fey Legacy
	                   if (GetSpellId() == SPELL_CONFUSION && !GetIsObjectValid(GetSpellCastItem()) && GetLastSpellCastClass() == CLASS_TYPE_INVALID)
	                   {
	                       nDuration = GetScaledDuration(GetHitDice(OBJECT_SELF), oTarget);
	                   }

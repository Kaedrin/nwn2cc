...
//Declare major variables
	...
	effect		eDam;
	effect eCold = EffectDamageResistance(DAMAGE_TYPE_COLD, 9999, 0);
	effect eElec = EffectDamageResistance(DAMAGE_TYPE_ELECTRICAL, 9999, 0);
	effect eFire = EffectDamageResistance(DAMAGE_TYPE_FIRE, 9999, 0);	
	...
	{	//FoF discrimination
		if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, oCaster) && oTarget != oCaster)
		{
			...
			if (!ResistSpell(oCaster, oTarget))
			{
				...
				if (MySavingThrow(SAVING_THROW_FORT, oTarget, GetSpellSaveDC(), nSaveThrow, oCaster))
				{
					nDam	=	nDam / 2;
				}

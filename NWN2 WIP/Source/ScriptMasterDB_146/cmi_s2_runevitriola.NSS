...
int GetAcidReserveDamageDice(object oCaster)
{

	if (GetHasSpell(173,oCaster))
		return 9;
	if (GetHasSpell(71,oCaster))
		return 7;	
	if (GetHasSpell(0,oCaster))
		return 6;	
	if (GetHasSpell(873,oCaster))
		return 5;	
	if (GetHasSpell(1859,oCaster) || GetHasSpell(1205,oCaster))
		return 4;		
	if ( GetHasSpell(523,oCaster) || GetHasSpell(1753,oCaster) || GetHasSpell(1814,oCaster) )
		return 3;															
...
void main()
{
    ...
    int nDamage;
    int nIce = GetLocalInt(OBJECT_SELF, "NW_SPELL_DELAY_RUNE_VITRIOL");
	
	int nDamageDice = GetAcidReserveDamageDice(oCaster);
	int nDC = GetReserveSpellSaveDC(nDamageDice,oCaster);
	nDamageDice = nDamageDice * 2;
				
    effect eDam;
    effect eExplode = EffectVisualEffect(VFX_RUNE_VITRIOL);
    ...
    if(nIce == 0)
    {
        ...
        {
            SetLocalInt(OBJECT_SELF, "NW_SPELL_DELAY_RUNE_VITRIOL",TRUE);

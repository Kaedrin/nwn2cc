...
void main()
{
   ...
   if (!GetHasFeat(FEAT_BARD_SONGS, OBJECT_SELF))
   {
	...
	if (nHex > 0)
	{
		if (GetHasFeat(FEAT_HEXBLADE_RHYME_MISFORTUNE))
		{
			if (nHex > nLevel) // Caps at Bardic level
				nHex = nLevel;
			nHex = nHex/2; // Only half applies	
			nLevel += nHex;
		}
	}
    ...
    effect eAttack;
    effect eDamage1, eDamage2, eDamage3;
    effect eWill;
    effect eFort;
    effect eReflex;
    effect eHP;
    ...
    eDamage1 = EffectDamageDecrease(nDamage, DAMAGE_TYPE_SLASHING);
    eDamage2 = EffectDamageDecrease(nDamage, DAMAGE_TYPE_BLUDGEONING);
    eDamage3 = EffectDamageDecrease(nDamage, DAMAGE_TYPE_PIERCING);		
    effect eLink = EffectLinkEffects(eAttack, eDamage1);

...
void HandleZeroCon(object oTarget)
{
	int nCon = GetAbilityScore(oTarget, ABILITY_CONSTITUTION);
	if (nCon <= 0)
	{
		effect eDeath = EffectDeath(FALSE, TRUE, TRUE); // kill regardless of death immunity
		DelayCommand(0.1f,  ApplyEffectToObject(DURATION_TYPE_INSTANT, eDeath, oTarget));
		SendMessageToPC(oTarget, "Cloudkill has slain you after draining your Constitution to zero.");		
	}
}
...
void main()
{
    ...
    eDam = ExtraordinaryEffect(eDam);
			...
			if (nDC >= 100)
			{
				nDC = GetLocalInt(GetAreaOfEffectCreator(), "DC23");
				if (nDC == 0)
					nDC = 18;
			}	
    ...
    if(spellsIsTarget(oTarget,SPELL_TARGET_STANDARDHOSTILE , GetAreaOfEffectCreator()) )
    {
        ...
        //if(!MyResistSpell(GetAreaOfEffectCreator(), oTarget, fDelay))
        {
            ...
            else if (nHD >= 4 && nHD <= 6)
            {
                ...
                if(!MySavingThrow(SAVING_THROW_FORT, oTarget, nDC, SAVING_THROW_TYPE_DEATH, OBJECT_SELF, fDelay))
                {
                    DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_INSTANT, eDeath, oTarget));
                    DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oTarget));
					SendMessageToPC(oTarget, "Cloudkill has slain you for being too weak.");		
                ...
                else
                {
					...
					SendMessageToPC(oTarget, "Cloudkill attempts to damage your Constitution.");
		...
		DelayCommand(fDelay + 0.1f, HandleZeroCon(oTarget));
    }
}
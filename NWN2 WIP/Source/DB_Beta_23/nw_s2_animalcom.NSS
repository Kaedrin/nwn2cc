...
void main()
{
	...
	if (GetHasFeat(FEAT_UNSTOPPABLE_FEROCITY))
	{
	...
	int nRegen = 1; //New passive Regen for all companions. 
	...
	//Heal the pet due to adding toughness and epic toughness
	
	if ( (nHD > 8) && (nElemental != TRUE) )
	{
		int nHeal = nHD;
		if (nHD > 23)
			nHeal += 30;
		effect eHeal =  EffectHeal(nHeal);	
		DelayCommand(2.0f, ApplyEffectToObject(DURATION_TYPE_INSTANT, eHeal, oMyPet));	
	}
	...
	if (nHD > 23)
	{
		FeatAdd(oMyPet, 754, FALSE); //Epic Toughness
		FeatAdd(oMyPet, 2128, FALSE); //Epic Precision
		FeatAdd(oMyPet, 408, FALSE); //Blind Fight	
		FeatAdd(oMyPet, 2170, FALSE); //Steadfast Determination	
		FeatAdd(oMyPet, 40, FALSE); //Toughness	
		FeatAdd(oMyPet, 291, FALSE); //Weapon Focus (Creature)	
		FeatAdd(oMyPet, 10, FALSE); //Dodge	
		FeatAdd(oMyPet, 382, FALSE); //Luck of Heroes	
	}
	else
	if (nHD > 20)
	{

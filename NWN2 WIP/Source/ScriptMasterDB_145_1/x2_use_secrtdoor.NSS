...
void main()
{
    ...
    if (!GetLocked(OBJECT_SELF))
    {
        if (GetIsOpen(OBJECT_SELF))
        {
			...
			// FalloutBoy 1/20/07: Fixed this for NWN 2, although I wonder
			//   if familars, etc. will get left behind.
			AssignCommand(oidDest, ActionOpenDoor(oidDest));
			object oFaction = GetFirstFactionMember( oidUser, FALSE );
			SendCreature(oidUser, oidDest);
			//while ( GetIsObjectValid( oFaction ) )
			//{
			//	SendCreature( oFaction, oidDest );
			//	oFaction = GetNextFactionMember( oidUser, FALSE );
			//}
			/*

...
//                  Jug_Debug(GetName(OBJECT_SELF) + " starting combat round in percep");
	...
	// If it's a PC and you can see it, mark
	// that a pc was seen
	if ((GetIsPC(oLastPerceived)) && (!GetIsDM(oLastPerceived))) {
		if ((GetObjectSeen(oLastPerceived)) && (GetLocalInt(OBJECT_SELF, PC_SEEN) == FALSE)) {
			SetLocalInt(OBJECT_SELF, PC_SEEN, TRUE);
			
			// Store when it was seen. when the attack takes place,
			// check against this time to see how much time 
			// has passed from seeing the PC to the attack itself.
			// If very little time passed, allow it to be killed.
			SetLocalInt(OBJECT_SELF, PC_SEEN_TIME, GetUNIXTime());
		}
	}
}
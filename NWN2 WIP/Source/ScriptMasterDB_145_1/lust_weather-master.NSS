...
void main()
{
	...
	int RainChances = GetLocalInt(OBJECT_SELF, "Rain_Frequency"); // int RainChances = GetLocalInt(OBJECT_SELF, "Rain_Frecuence");
	int RainMaxPower = GetLocalInt(OBJECT_SELF, "Rain_MaxPower");
	if (RainChances==0) RainChances=0;
	...
	//SendMessageToPC(GetFirstPC(), "Rainpower: "+IntToString(GetWeather(oArea, WEATHER_TYPE_RAIN))+" for: "+IntToString(GetLocalInt(OBJECT_SELF, "WeatherRounds"))+" rounds.");
}

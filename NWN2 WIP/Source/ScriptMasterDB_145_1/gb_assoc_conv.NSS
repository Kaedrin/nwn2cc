...
void main()
{
	...
	// Ella's goblin pet addition
	if ((oMaster == oShouter) && (GetTag(OBJECT_SELF) == "hv_pet_goblin")) {

		if ((nMatch == 2001) || (nMatch == 2002))
   		{
   			DelayCommand(2.0, AssignCommand(OBJECT_SELF, SpeakString("<C=orange>Commin' mastah!")));
    		DelayCommand(2.0, AssignCommand(OBJECT_SELF, ActionForceMoveToObject(oMaster, TRUE, 0.5f)));
	    	DelayCommand(2.0, AssignCommand(OBJECT_SELF, ActionStartConversation(oMaster,"",TRUE,FALSE,FALSE,FALSE)));
   		}
	}
}
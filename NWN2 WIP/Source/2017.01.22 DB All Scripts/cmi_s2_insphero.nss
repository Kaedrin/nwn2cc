...
void RunPersistentSong(object oCaster, int nSpellId)
{
	...
	int nImprovedHeroics = GetHasFeat(FEAT_SKALD_IMPROVED_INSPIRE_HEROICS);	
	int nGreaterHeroics = GetHasFeat(FEAT_SKALD_GREATER_INSPIRE_HEROICS);
    ...
    //if(nSingingSpellId == nSpellId)
    {
		...
		{
			nBonus = nSkald/5;
			effect eWill = EffectSavingThrowIncrease(SAVING_THROW_WILL, nBonus);
			eLink = EffectLinkEffects(eLink, eWill);
		}
		if (nInspireToughness)
		{
			effect eDmgImm1 = EffectDamageImmunityIncrease(DAMAGE_TYPE_BLUDGEONING, nSkald/2);
			effect eDmgImm2 = EffectDamageImmunityIncrease(DAMAGE_TYPE_PIERCING, nSkald/2);
			effect eDmgImm3 = EffectDamageImmunityIncrease(DAMAGE_TYPE_SLASHING, nSkald/2);
			eLink = EffectLinkEffects(eLink, eDmgImm1);
			eLink = EffectLinkEffects(eLink, eDmgImm2);
			eLink = EffectLinkEffects(eLink, eDmgImm3);
		}
	
		// Health, now integrated into the base ability
		{	
			nBonus = 1;	
		...
		{	
			...
			effect eDmgBoost = EffectDamageIncrease(nBonus);
			eLink = EffectLinkEffects(eLink, eDmgBoost);
		}	

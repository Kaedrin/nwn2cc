...
void main()
{
	...
	effect eOrbEffect = EffectSlow();
	int nSaveThrow = SAVING_THROW_TYPE_ELECTRICITY;	
         ...
         while (GetIsObjectValid(oTarget))
        {
           ...
           if (oTarget != OBJECT_SELF && oTarget2 == oTarget)
           {
                if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF))
            	{
                    ...
                    //Set damage effect
                    eDamage = EffectDamage(nDamage, DAMAGE_TYPE_ELECTRICAL, DAMAGE_POWER_NORMAL, bIgnoreResists);

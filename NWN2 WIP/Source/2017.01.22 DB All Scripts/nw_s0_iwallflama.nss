...
void main()
{
    ...
    if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, GetAreaOfEffectCreator()))
    {
        ...
        if(!MyResistInvocation(GetAreaOfEffectCreator(), oTarget))
        {
            //Roll damage.
            ...
            if ( GetRacialType(oTarget) == RACIAL_TYPE_UNDEAD )
            {
                ...
                //nFireDamage = GetReflexAdjustedDamage(nFireDamage, oTarget, GetWarlockDC(oCreator, TRUE), SAVING_THROW_TYPE_FIRE);
                if(nFireDamage > 0)
                {

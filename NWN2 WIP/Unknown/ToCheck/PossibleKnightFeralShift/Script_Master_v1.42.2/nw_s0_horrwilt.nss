        ...
        if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF) && oTarget != OBJECT_SELF)
        {
            ...
            if (!MyResistSpell(OBJECT_SELF, oTarget))
            {
                if(GetRacialType(oTarget) != RACIAL_TYPE_CONSTRUCT && GetRacialType(oTarget) != RACIAL_TYPE_UNDEAD)
                {
                    ...
                    if(/*Fort Save*/ MySavingThrow(SAVING_THROW_FORT, oTarget, GetSpellSaveDC(), SAVING_THROW_TYPE_NONE, OBJECT_SELF))
                    {
                        nDamage = nDamage/2;
                    }
                    ...
                    DelayCommand(0.1f, ApplyEffectToObject(DURATION_TYPE_INSTANT, eDam, oTarget));

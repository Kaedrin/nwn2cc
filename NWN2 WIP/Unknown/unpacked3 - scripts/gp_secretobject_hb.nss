...
// 1/20/07 FalloutBoy
//   Fixed door functionality for NWN2
...
int CheckIfDetected(object oSearcher)
{
	...
	int nRoll, nDC;
	//string sItem=GetLocalString(OBJECT_SELF, "Item_Required");
	if (nDetectMode)
	{
	...
	//Mustang add for item check
	//if(sItem != "" && GetIsObjectValid(GetItemPossessedBy(oSearcher, sItem))==FALSE)
	//{
	//	return 0;
	//}
	//THE CHECK!
...
void RevealObject()
{
	...
	if (sJumpToTag!="")	
	{
		// FalloutBoy 1/20/07: Fixed these 3 lines for secret doors in NWN 2
		//object oDest=GetObjectByTag(sJumpToTag);
		SetLocalString(oSpawn,"Destination",sJumpToTag);
		SetEventHandler( oSpawn, SCRIPT_PLACEABLE_ON_USED, "x2_use_secrtdoor");
	...
	// FalloutBoy 1/20/07: If not resetting, then we are done
	if (fTimeToReset == 0.0f)

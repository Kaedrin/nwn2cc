...

{
	...
	int nRichard = GetJournalEntry("Richard", oPC);
	
	if (nRichard == 210 && GetObjectByTag("hv_talonakid")==OBJECT_INVALID) 
	{
		object oRWP = GetObjectByTag("richard_wp");
		location lLocation = GetLocation(oRWP);
		CreateObject(OBJECT_TYPE_CREATURE, "hv_talonakid", lLocation);
	}

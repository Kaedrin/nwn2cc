...
// Determines if equipped item has bonus hitpoints and returns amount, returns 0 if none
int GetBonusHP(object oItem)
{
	int nHPBonus = 0;
	itemproperty ipItem = GetFirstItemProperty(oItem);
	while(GetIsItemPropertyValid(ipItem))
	{			
		if(GetItemPropertyType(ipItem) == ITEM_PROPERTY_BONUS_HITPOINTS)
		{
			int nHPBonusSubType = GetItemPropertyCostTableValue(ipItem);
			if(nHPBonusSubType <= 20)
			{
				nHPBonus += nHPBonusSubType;
			}else
			{
				if(nHPBonusSubType == 21)
				{
					nHPBonus += 25;
				}
				if(nHPBonusSubType == 22)
				{
					nHPBonus += 30;
				}
				if(nHPBonusSubType == 23)
				{
					nHPBonus += 35;		
				}
				if(nHPBonusSubType == 24)
				{
					nHPBonus += 40;
				}
				if(nHPBonusSubType == 25)
				{
					nHPBonus += 45;
				}
				if(nHPBonusSubType == 26)
				{
					nHPBonus += 50;
				}
			}
...
void FixBonusHP(object oItem, object oPC)
{
	// Corrects Bonus HP
	int nBonusHP = GetBonusHP(oItem);
	if(nBonusHP > 0 && (GetCurrentHitPoints(oPC) > GetMaxHitPoints(oPC)))
	{
		if(nBonusHP > GetCurrentHitPoints(oPC))
		{
			// Prevents PC from being killed ( well mostly, lol )
			nBonusHP = GetCurrentHitPoints(oPC) - 1;
		}else
		{			
			// Damages PC equal to the number of hit points they are over max, but less than the total number of bonus hit points.  
			int nOverageHP = GetCurrentHitPoints(oPC) - GetMaxHitPoints(oPC);
			if(nOverageHP < nBonusHP )
			{
				nBonusHP = nOverageHP;
			}
		}		
		effect eDamage = EffectDamage(nBonusHP, DAMAGE_TYPE_POSITIVE, DAMAGE_POWER_NORMAL, TRUE);
		DelayCommand(0.2f,ApplyEffectToObject(DURATION_TYPE_INSTANT, eDamage, oPC)); 				
		SendMessageToPC(oPC, "Your Hit Points have been corrected due to unequipping an item with a Bonus Hit Points item property.");
	}			
}
...
//	}
	...
	if (GetLocalInt(GetModule(), "UseSRFix"))
		DelayCommand(0.3f, ApplySRFixOnUnequip(oPC, oItem));
	...
	if (GetHasFeat(FEAT_COMBATSTYLE_RANGER_TWO_HANDED, oPC))
	{
		object oArmor = GetItemInSlot(INVENTORY_SLOT_CHEST, oPC);
		if ((oArmor != OBJECT_INVALID) && (GetArmorRank(oArmor) > ARMOR_RANK_LIGHT) )
		{
			//Penalty Valid
			if (!GetHasSpellEffect(SPELLABILITY_COMBATSTYLE_RANGER_TWO_HANDED, oPC))
			{			
				effect eLink = EffectAttackDecrease(4);		
				eLink = SetEffectSpellId(eLink,SPELLABILITY_COMBATSTYLE_RANGER_TWO_HANDED);
		...
		else 
		{
			...
			if (GetHasSpellEffect(SPELLABILITY_COMBATSTYLE_RANGER_TWO_HANDED, oPC))
			{
				RemoveEffectsFromSpell(oPC, SPELLABILITY_COMBATSTYLE_RANGER_TWO_HANDED);					
			}
		}
	}
	
	if (GetHasFeat(FEAT_FACTOTUM_CUN_DEF, oPC))
		DelayCommand(0.1f, EvaluateCunningDefense(oPC));
	if (GetHasFeat(FEAT_FIGHTER_ARMOR_TRAIN, oPC))
		DelayCommand(0.1f, EvaluateFighterArmorTrain(oPC));			
	
	DelayCommand(0.7f, FixBonusHP(oItem, oPC));			

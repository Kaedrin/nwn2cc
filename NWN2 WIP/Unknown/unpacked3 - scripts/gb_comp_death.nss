...
void main()
{
	...
	object oPC = GetControlledCharacter(GetFirstPC());
	...
	{		
		...
		{			
			if ( GetIsPartyPossessible(oDead) == FALSE && GetPartyMembersDyingFlag() == TRUE)
			{
				SendMessageToPC(oPC, "gb_comp_death: no party member possessiable");		

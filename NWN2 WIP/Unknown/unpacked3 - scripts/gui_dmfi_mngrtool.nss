...
void main(string sValue, int nObj, float nX, float nY, float nZ)
{
	...
	string sDMKey = GetPCPublicCDKey(OBJECT_SELF);
...
//	if (!GetIsDM(oPC)) return;
	...
	if (GetStringLeft(sValue,5)=="SLOT_")
		...
		StoreCampaignObject(DMFI_STORE_DB,"SLOT_"+IntToString(nString) + "_" + sDMKey,oTarget,oPC);
	...
	else if (sValue == "BUT_PICKUP")
		{
		 ...
		 if(GetObjectType(oLoc)==1)
		 	 oLod = RetrieveCampaignObject(DMFI_STORE_DB,"SLOT_"+IntToString(nString) + "_" + sDMKey,GetLocation(oLoc),oLoc);
			else
			 oLod = RetrieveCampaignObject(DMFI_STORE_DB,"SLOT_"+IntToString(nString) + "_" + sDMKey,lLoc);

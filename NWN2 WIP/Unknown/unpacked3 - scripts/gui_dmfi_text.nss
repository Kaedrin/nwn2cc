...
void main (string sInput)
{
	...
	if (sInput!="")
	{
		// Ella change - check if whisper
		if (GetSubString(sTest, 0, 2) == "/w") {
			sTest = GetSubString(sTest, 2, GetStringLength(sTest) - 2);
			AssignCommand(oTarget, SpeakString(sTest, TALKVOLUME_WHISPER));
		}
		else {

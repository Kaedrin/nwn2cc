...
void main()
{
	...
	// Ella's addition to default script - 
	// if challenger spotted, make her enemy
	object oPC = GetLastPerceived();
	if (GetTag(GetArea(OBJECT_SELF)) == "hv_arena") {
		int nCurrentRep = GetReputation(OBJECT_SELF, oPC);
		if (GetLocalInt(oPC, ARENA_CHALLENGE_VAR) == 1) {
			//if (nCurrentRep == 50) {
				AdjustReputation(oPC, OBJECT_SELF, -100);
			//}
		}
		else { // not a challenger anymore
			if (nCurrentRep < 50) {

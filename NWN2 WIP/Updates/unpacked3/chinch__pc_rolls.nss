...
int StartingConditional()
{
 ...
 while (nRoll <= 3 && sMsg == "")
 {
  ...
  if (nDie1 == nDie2 && nDie1 == nDie3) //Check Storms
  {
   if (nDie1 == 1)
   {
    sMsg = sNoScore + "You have rolled a " + IntToString(nDie1) + ", " + IntToString(nDie2) + ", and " + IntToString(nDie3) + ". It's a Storm in my favor! I win three times the bet.";
    TakeGoldFromCreature(3 * pBet, oPC);
   }
   else
   {
    sMsg = sNoScore + "You have rolled a " + IntToString(nDie1) + ", " + IntToString(nDie2) + ", and " + IntToString(nDie3) + ". It's a Storm in your favor! You win three times the bet.";
    GiveGoldToCreature(oPC, 3 * pBet);
   }
   DeleteLocalInt(oPC, "pBet");
   DeleteLocalInt(oPC, "Score");
  }
  else if (nDie1 == nDie2 || nDie1 == nDie3 || nDie2 == nDie3) //Check Doubles
  {
   nScore = nDie1 == nDie2 || nDie1 == nDie3?(nDie2 == nDie1?nDie3:nDie2):nDie1;
   sMsg = sNoScore + "You have rolled a " + IntToString(nDie1) +", " + IntToString(nDie2) + ", and " + IntToString(nDie3) + ". Your score is " + IntToString(nScore) +".";
   if (nScore > nNPCScore)
   {
    sMsg += "\nYou win.";
    GiveGoldToCreature(oPC, pBet);
   }
   else if (nScore < nNPCScore)
   {
    sMsg += "\nI win.";
    TakeGoldFromCreature(pBet, oPC);
   }
   else sMsg += "\nIt's a draw.";
   DeleteLocalInt(oPC, "pBet");
   DeleteLocalInt(oPC, "Score");
  ...
  {
   sMsg = sNoScore + "You have rolled a 1, 2, and 3. I win two times the bet.";
  ...
  {
   sMsg = sNoScore + "You have rolled a 4, 5, and 6. You win two times the bet.";
  ...
  {
   sNoScore += "You have rolled a " + IntToString(nDie1) + ", " + IntToString(nDie2) + ", and " + IntToString(nDie3) + ". No score.\n";
  }
  nRoll++;
 }
 if (sMsg == "")
